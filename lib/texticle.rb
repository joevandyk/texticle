...
module Texticle
  ...
  def search(query = "", exclusive = true)
    ...
    query.each do |column, search_term|
      ...
      similarities << "ts_rank(to_tsvector(#{language}, #{quoted_table_name}.#{column}::text), plainto_tsquery(#{language}, #{search_term}::text))"
      conditions << "to_tsvector(#{language}, #{quoted_table_name}.#{column}::text) @@ plainto_tsquery(#{language}, #{search_term}::text)"
    end
